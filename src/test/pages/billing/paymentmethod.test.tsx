...
describe('PaymentMethod Component', () => {
    const mockCards = [
  {
    ...
    Default: false,
    created: Date.now(), // Use a timestamp (number) instead of a string
  },
...
//     // Add assertions based on the expected behavior
//     // For example, you can check if the ConfirmationDialog is opened
//     expect(screen.getByText(/are you sure/i)).toBeInTheDocument();
//   });
...
//   });
  ...
  test('handles ConfirmationDialog onConfirm', async () => {
    ...
    // Wait for async operations to complete (e.g., API call)
    await waitFor(() => {
      // Add assertions based on the expected behavior after confirmation
